"""Tests for North Carolina public notice collector."""
...
def _load_fixture():
    return json.loads((FIXTURES_DIR / "north_carolina_notices.json").read_text())


def _mock_scrape_result(data):
    return ScrapeResult(
        fetch=FetchResult(
            url="https://www.ncnotices.com",
            status_code=200,
            fetcher="crawl4ai",
...
@pytest.fixture()
def collector():
    return NorthCarolinaCollector()


def test_name(collector):
    assert collector.name == "north_carolina_public_notice"
...
def test_normalize_valid_record(collector):
    raw = {"county": "Wake", "sale_date": "2026-07-10", "sale_type": "deed"}
    auction = collector.normalize(raw)
    assert auction.state == "NC"
    assert auction.county == "Wake"
...
async def test_fetch_returns_auctions(collector):
    ...
    assert all(a.state == "NC" for a in auctions)
