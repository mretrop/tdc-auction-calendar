"""Tests for South Carolina public notice collector."""
...
def _load_fixture():
    return json.loads((FIXTURES_DIR / "south_carolina_notices.json").read_text())


def _mock_scrape_result(data):
    return ScrapeResult(
        fetch=FetchResult(
            url="https://www.scpublicnotices.com",
            status_code=200,
            fetcher="crawl4ai",
...
@pytest.fixture()
def collector():
    return SouthCarolinaCollector()


def test_name(collector):
    assert collector.name == "south_carolina_public_notice"
...
def test_normalize_valid_record(collector):
    raw = {"county": "Charleston", "sale_date": "2026-10-05", "sale_type": "deed"}
    auction = collector.normalize(raw)
    assert auction.state == "SC"
    assert auction.county == "Charleston"
...
async def test_fetch_returns_auctions(collector):
    ...
    assert all(a.state == "SC" for a in auctions)
